...
**Summary** — Added `findTaskFileRenames` and `handleTaskFileDone` helpers to the GitHub webhook handler in `svelte-todo-kanban`. When a push renames a `NNN-*-TODO.md` to `NNN-*-DONE.md`, the webhook: fetches the DONE file to extract the card UUID, loads the todo, checks idempotency (skips if card is already past TODO), moves the card to the Review list, posts a completion comment, and closes the linked GitHub issue.

**Files changed**
- `svelte-todo-kanban/src/lib/graphql/documents.ts` — added `GET_TODO_BY_ID` (typed) and `GET_BOARD_BY_REPO` (raw string, jsonb _contains bypass) queries
- `svelte-todo-kanban/src/routes/api/github/webhook/+server.ts` — added `findTaskFileRenames`, `handleTaskFileDone`, wired loop into `handlePushEvent`

**Verification**
- `npm run generate` ✅ (codegen passes)
- `npm run check` ✅ (no errors in webhook file; 20 pre-existing errors in unrelated files unchanged)
- `npm run test:unit:server` ✅ (138/138 tests pass)

**Deviations**
- `GET_BOARD_BY_REPO` uses a raw string instead of `graphql()` because the `github` column is typed as `String` in the generated schema (not `jsonb`), so `_contains` fails codegen validation — same pattern used by existing code in the file.
