...
See ../server server "Karel" and I have already installed there Herdr, cloned all the repos in `/plugins/dev-kit/runner/config.json` in `kasparpalgi/klarity-claude-kit` repo runner, claude code (give me URL to authenticate once time is there) and everything working in this machine (same config/repos must be cloned there) and then I shall be able to pick from the board which one to run.\
\
