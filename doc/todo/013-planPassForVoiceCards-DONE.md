...
**Summary** — Built server-side plan pass for long voice-dumped cards. A new `/api/ai/plan` endpoint in svelte-todo-kanban strips HTML from the card content, checks if it exceeds 300 chars of plain text, and if so calls OpenAI to produce a structured task description (same language as original). The endpoint returns the rewritten HTML body with the raw voice note preserved under `## Raw voice note`. Idempotency is enforced by a `<!-- planned -->` marker prepended to content. The trigger fires non-blocking from `CardDetailView.saveTodo()` whenever a save succeeds with long, unplanned content.

**Files changed**
- `src/routes/api/ai/plan/+server.ts` — created; POST endpoint: HTML→text, length gate, `<!-- planned -->` idempotency guard, OpenAI call, returns rewritten content
- `src/lib/components/todo/CardDetailView.svelte` — added `runPlanPass()` helper and non-blocking trigger after successful save
- `package.json` — bumped to `0.9.0`

**Verification**
- [x] Short card (<300 chars plain text) returned unchanged — length gate exits early
- [x] Long card gets `<!-- planned -->` + structured HTML + `## Raw voice note` + plain text
- [x] Same card not planned twice — both server-side (`content.includes(PLANNED_MARKER)`) and client-side guard
- [x] Language preserved — prompt explicitly instructs: "Keep the SAME LANGUAGE as the voice note. Do NOT translate."
- [x] `npm run check` — no new errors introduced (19 pre-existing errors in unrelated files)
- [ ] Item 4 from original checklist ("/plan task file in right repo") is N/A — design updated per second-pass note to use server-side OpenAI correction rather than Claude CLI

**Deviations**
- Runner-based design from the original task was superseded by the second-pass note: this lives in svelte-todo-kanban (server-side), not in `plugins/dev-kit/runner/`
- No board-setting gate for "Backlog only" — trigger fires on any long saved content. The Backlog restriction requires board settings (a 014-level addition); the idempotency marker prevents double-processing regardless
- Used `gpt-5-mini` (consistent with existing AI endpoints) rather than `claude -p /plan` (which would require Claude CLI server-side)
